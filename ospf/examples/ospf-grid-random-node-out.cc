...
SetLinkDown (Ptr<NetDevice> nd)
{
  ...
  pem->SetRate (1.0);
  Ptr<NetDevice> rnd;
  rnd = nd->GetChannel ()->GetDevice (0) == nd ? nd->GetChannel ()->GetDevice (1)
                                               : nd->GetChannel ()->GetDevice (0);
  // bi-directional
  nd->SetAttribute ("ReceiveErrorModel", PointerValue (pem));
  rnd->SetAttribute ("ReceiveErrorModel", PointerValue (pem));
}
...
main (int argc, char *argv[])
{
  ...
  LogComponentEnable ("OspfGridPartition", LOG_LEVEL_INFO);
  ...
  rv->SetAttribute ("Max", DoubleValue (GRID_HEIGHT * GRID_WIDTH - 1));
  ...
  // Test Error
  for (uint32_t i = 10; i < SIM_SECONDS; i += 100)
    {
      uint32_t j = ((int) rv->GetValue ()) % (GRID_HEIGHT * GRID_WIDTH);
      std::cout << j << std::endl;
      Simulator::Schedule (Seconds (i), &SetLinkDown, c.Get (j)->GetDevice (1));
      Simulator::Schedule (Seconds (i), &SetLinkDown, c.Get (j)->GetDevice (2));
      Simulator::Schedule (Seconds (i), &SetLinkDown, c.Get (j)->GetDevice (3));
      Simulator::Schedule (Seconds (i), &SetLinkDown, c.Get (j)->GetDevice (4));
      Simulator::Schedule (Seconds (i + 50), &SetLinkUp, c.Get (j)->GetDevice (1));
      Simulator::Schedule (Seconds (i + 50), &SetLinkUp, c.Get (j)->GetDevice (2));
      Simulator::Schedule (Seconds (i + 50), &SetLinkUp, c.Get (j)->GetDevice (3));
      Simulator::Schedule (Seconds (i + 50), &SetLinkUp, c.Get (j)->GetDevice (4));
      Simulator::Schedule (Seconds (i + 49), &CompareLsdb, c);
      Simulator::Schedule (Seconds (i + 99), &CompareLsdb, c);
    }
